...
#RUn History
#syncbench-mbank-300cycles-nopfixed-4 ->initial run after fixing rp writeback delays
#syncbench-mbank-300cycles-nopfixed-4-1 ->same
#syncbench-mbank-300cycles-nopfixed-4-2 ->add more counters for write backs and epoch sizes, add thread 1,

CONF_NAME=syncbench-mbank-300cycles-nopfixed-5-2_last_100
...
do
	...
	do
			...
			for bench in linkedlist hashmap bstree skiplist-rotating-c lfqueue
			...
			do	
				...
				do
					...
					${HOME}/repos/primesim/xml/config_mbank_300cycles_${pmodel}.xml \
					...
					-c ${HOME}/repos/primesim/xml/config_mbank_300cycles_${pmodel}.xml \
					-o ${HOME}/repos/primesim/output/${CONF_NAME}/config_${threads}_${rate}_${rate2}_${operations}_${bench}_${pmodel}.out \
					-- ${HOME}/repos/primesim/pbench/syncbench/bin/${bench} -t ${threads} -i ${rate} -r ${rate2} -o ${operations} -d 10 -x 6 -u 100
