...
int main(int argc, char **argv)
{
	...
	while(1) {
		i = 0;
		c = getopt_long(argc, argv, "hAf:d:i:t:r:S:u:a:s:l:x:o:", long_options, &i);
	...
	*/	
	...
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
		...
		data[0].load_factor = load_factor;
		data[0].move = move;
		data[0].snapshot = snapshot;
		...
		data[0].nb_move = 0;
		data[0].nb_moved = 0;
		data[0].nb_snapshot = 0;
		data[0].nb_snapshoted = 0;
